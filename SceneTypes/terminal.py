...
class TerminalScene(Scene):
    ...
    def ls(self, save_key: str, map: dict, visible: bool, dic_save:dict) -> list:
        """
        entrée :
    ...
    def cd(self, save_key: str, argument: str, possibilities: list, dic_save:dict) ->None:
        """
        entrée :
        ...
        if possibilities != None :
            verif_arg = self.search(possibilities, argument)
        else :
            verif_arg = False
        #----------------------------------------------
    ...
    def search(self, liste, platform) -> bool:
        """
        entrée:
    ...
    def Terminal(self, save_key, dic_save:dict) -> None:
        ...
        while True:
            ...
            if cmd == "exit":  #sortie du Terminal
                ...
                possibilities = self.ls(save_key, map, False)
                self.cd(save_key, argument, possibilities)

            elif cmd == "-h":  #affiche toutes les commandes
                for i in self.dic_cmd.keys():
                    time.sleep(0.1)
                    print(f" { i } : { self.dic_cmd[i] } \n")
